@include('admin.header')

        <div class="container">
            <div class="row">
                <div class="col">
                    <p class="text-white mt-5 mb-5">Welcome back, <b>{{Auth::user()->name}}</b></p>
                </div>
            </div>
            <!-- row -->
            <div class="row tm-content-row">
                <div class="col-12 tm-block-col">
                    <div>
                        <h2 class="tm-block-title">Users</h2>
                        @if(session()->has('success'))
                        <div class="alert alert-success alert-block d-flex justify-content-between">
                            <div> <strong>{{ session()->get('success') }}
                                </strong> </div>
                            <div><button type="button" class="close" data-dismiss="alert">×</button></div>
                        </div>
                        @endif
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">S.No</th>
                                    <th scope="col">USER NAME</th>
                                    <th scope="col">STATUS</th>
                                    <th scope="col">OPERATORS</th>
                                    <th scope="col">ROLES</th>
                                </tr>
                            </thead>
                            <tbody>
                                @php
                                    $serial_number=1;
                                @endphp

                                @foreach($datas as $data)
                                <tr>
                                    <th scope="row"><b>{{$serial_number++}}</b></th>
                                    <td><b>{{$data->name}}</b></td>
                                    <td>
                                        <div class="tm-status-circle moving">
                                        </div>{{$data->status}}
                                    </td>
                                    @if($data->status =='Active')
                                    <td><a href="{{route ('admin.ChangeStatus',$data->id) }}" class="btn btn-primary text-uppercase">Inactive</a></td>
                                    @else
                                    <td><a href="{{route ('admin.ChangeStatus',$data->id) }}" class="btn btn-primary text-uppercase">Active</a></td>
                                    @endif

                                    <td><b>{{$data->role->name}}</b></td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
        <footer class="tm-footer row tm-mt-small">
            <div class="col-12 font-weight-light">
                <p class="text-center text-white mb-0 px-4 small">
                    Copyright &copy; <b>2018</b> All rights reserved. 
                    
                    Design: <a rel="nofollow noopener" href="https://templatemo.com" class="tm-footer-link">Template Mo</a>
                </p>
            </div>
        </footer>
    </div>
</body>

@include('admin.footer')